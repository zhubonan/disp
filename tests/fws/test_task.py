...
need_gulp = pytest.mark.skipif(  # pylint: disable=invalid-name
    not (has_exe("gulp_relax") and has_exe("gulp")) or bool(os.environ.get("CI")), reason="GULP is not avalaible"
)
need_castep = pytest.mark.skipif(  # pylint: disable=invalid-name
    not has_exe("castep.mpi") or bool(os.environ.get("CI")), reason="CASTEP is not avalaible"
)
need_buildcell = pytest.mark.skipif(not has_exe("buildcell"), reason="Buildcell is not avalaible")  # pylint: disable=invalid-name

need_pp3 = pytest.mark.skipif(not has_exe("pp3"), reason="Pp3 is not avalaible")  # pylint: disable=invalid-name
...
def test_buildcell_task(clean_launchpad, temp_workdir, get_data_dir, new_db, datapath):
    ...
    btask = AirssBuildcellTask(seed_content=seed_content, seed_name="C2", deposit_init_structure=True, project_name="TEST/C")
    ...
    Path("C2.cell").unlink()
    btask = AirssBuildcellTask(seed_content=seed_content, seed_name="C2", keep_seed=False, project_name="TEST/C")
    fwk = Firework([btask])
    wkf = Workflow.from_Firework(fwk)
    lpd = clean_launchpad
    lpd.add_wf(wkf)
...
def test_airss_validation_task(clean_launchpad, temp_workdir):
    ...
    with backup_path() as current_path:
        ...
        # Remove AIRSS related stuff in the PATH
        new_paths = ":".join([path for path in paths if "airss" not in path.lower()])
...
def test_gulp_relax_task(clean_launchpad, temp_workdir, get_data_dir):
    ...
    assert lpd.get_launch_by_id(1).action.stored_data["relax_status"] == RelaxOutcome.FINISHED.name
...
def test_full_gulp(clean_launchpad, temp_workdir, get_data_dir):
    ...
    btask = AirssBuildcellTask(seed_content=seed_content, seed_name="C2", project_name="TEST/C2")
...
def test_full_gulp_comb(clean_launchpad, temp_workdir, get_data_dir):
    ...
    btask = AirssBuildcellTask(seed_content=seed_content, seed_name="C2", store_content=False, project_name="TEST/C2")
    rtask = AirssGulpRelaxTask(
        param_content=param_content,
        cycles=100,
        executable="gulp",
...
def test_pp3_relax(clean_launchpad, temp_workdir, get_data_dir):
    ...
    btask = AirssBuildcellTask(seed_content=seed_content, seed_name="Al", store_content=False, project_name="TEST/Al")
...
def test_data_transfer(clean_launchpad, temp_workdir):
    ...
    # Test the keep mode
    task = AirssDataTransferTask(base_path=temp_workdir, keep=True, additional_types=[".castep"])
    fwt = Firework([task], spec={"struct_name": "C2-TEST", "project_name": "tests/c2/run2"})
    wkm = Workflow([fwt])
    clean_launchpad.add_wf(wkm)
    launch_rocket(clean_launchpad)
...
def test_insufficient_time(clean_launchpad, temp_workdir, get_data_dir):
    ...
    assert action.stored_data["relax_status"] == RelaxOutcome.INSUFFICIENT_TIME.name
...
def test_record_upload(temp_workdir, clean_launchpad, new_db, datapath):
    ...
    touch = lambda x: (Path(temp_workdir) / x).touch()
    for name in [seed_name + ".cell", struct_name + ".cell", struct_name + ".res"]:
        touch(name)
    spec = {"struct_name": struct_name, "project_name": project_name, "seed_name": seed_name, "db_file": str(datapath / "disp_db.yaml")}
    ...
    clean_launchpad.add_wf(Firework([task], spec=spec))
    launch_rocket(clean_launchpad)
    results = sdb.retrieve_project(project_name, include_seed=True, additional_filters={"struct_name": "C2-TEST-2"})
...
def test_clean_dir(temp_workdir, clean_launchpad):
    ...
    struct_name = "C2-TEST-1"
    touch = lambda x: (Path(temp_workdir) / x).touch()
    for name in [seed_name + ".cell", struct_name + ".cell", struct_name + ".res", seed_name + ".foo"]:
        touch(name)
    spec = {
        "clean_dir": True,
...
@need_pp3
def test_upload_record_task(clean_launchpad, temp_workdir, get_data_dir, new_db, datapath):
    """Test the buildcell task followed by gulp, packaged in the same FW"""
    ...
    assert len(list(temp_workdir.glob("Al-*.res"))) == 1
    results = new_db.retrieve_project(project_name="TEST/Al", include_seed=True, include_param=True)
...
def test_castep_code_selection(clean_launchpad, temp_workdir, get_data_dir):
    ...
    subprocess.run("ln -s `which castep.mpi` `pwd`/castep", shell=True, check=True)
    pwd = Path().resolve()
    worker = FWorker(env={"castep_codes": {"1911": f"mpirun -np 2 {pwd}/castep"}})
    wkf = Workflow([fwk])
    lpd.add_wf(wkf)
    ...
    assert Path("C2-TEST.castep").is_file()
    assert len(list(temp_workdir.glob("C2-TEST.res"))) == 1
    assert lpd.get_launch_by_id(2).action.stored_data["relax_status"] == RelaxOutcome.FINISHED.name
...
def test_castep_relax_script(clean_launchpad, temp_workdir, get_data_dir):
    ...
    spec = {
        ...
        "seed_name": "C2",
        "pressure": 100,
        "_fw_env": {"castep_relax_prepend_command": "module load essential"},
    }
    btask = AirssCastepRelaxTask(
        param_content=param_content,
        cycles=100,
    ...
    assert btask._get_cmd() == ["castep_relax", "100", "mpirun -np 2 castep.mpi", "0", "0", "C2-TEST"]

    # Prepare the run script
    btask._prepare_run_script(btask._get_cmd(), "module load essential", ["module unload essential", "echo all good"])
    ...
    assert content[4] == "'castep_relax' '100' 'mpirun -np 2 castep.mpi' '0' '0' 'C2-TEST'"
