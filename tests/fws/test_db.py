...
def test_insert_seed(clean_db, seed):
    ...
    seed1 = clean_db.insert_seed(project_name="test/run1", seed_name="C10", seed_content=seed)
    seed2 = clean_db.insert_seed(project_name="test/run1", seed_name="C10", seed_content=seed)
...
def test_insert_param(clean_db, param):
    """Test insertion of seed document"""
    param1 = clean_db.insert_param(project_name="test/run1", param_content=param, seed_name="TEST")
    param2 = clean_db.insert_param(project_name="test/run1", param_content=param, seed_name="TEST")
...
"""
    ...
    clean_db.insert_search_record(
        project_name="test/run1", struct_name="C10-TEST-1", res_content=res, param_content=param, seed_name="C10", seed_content=seed
    )

    clean_db.insert_search_record(
        project_name="test/run1",
    ...
    results = clean_db.retrieve_project("test/run1", include_seed=True, include_param=True)
    # Check relavent field are populated
    assert results[0].param_file.content
    assert results[0].seed_file.content

    results = clean_db.retrieve_project("test/run1", include_seed=False, include_param=False)
...
"""
    init = seed + "init"
    clean_db.insert_initial_structure(
        project_name="test/run1", struct_name="C10-TEST-1", seed_name="C10", struct_content=init, seed_content=seed
    )
    ...
    results = clean_db.retrieve_project("test/run1", include_seed=True, include_param=True, include_initial_structure=True)
    # Check relavent field are populated
    assert results[0].param_file.content
    assert results[0].seed_file.content
