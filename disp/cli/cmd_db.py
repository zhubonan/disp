...
def common_query_options(func):
    ...
    func = click.option('--state',
                        help='State of the FW',
                        type=click.Choice(list(Firework.STATE_RANKS)))(func)
...
@click.option('--state',
              multiple=True,
              type=click.Choice(list(Firework.STATE_RANKS)),
              help='Only include matches with these states')
@click.option('--show-priority',
              default=False,
              is_flag=True,
              help='Show the priority of workflows instead')
@click.option('--per-project',
              help='Summarise per project',
              is_flag=True,
              default=False)
@click.option('--no-res',
              is_flag=True,
              help='Do not query the search structure counts',
              default=False)
@click.option('--atomate',
              '-ato',
              is_flag=True,
              default=False,
...
def summary(db_obj, project, state, seed, per_project, workflows,
    ...
    """
    import pandas as pd
    df = db_obj.show_struct_counts(project,
                                   seed,
                                   state,
    ...
    with pd.option_context('display.max_rows', 99999, 'display.max_colwidth',
                           120):
        print(df)
...
@click.option('--group-by',
              ...
              type=click.Choice(
                  ['seed_name', 'project_name', 'worker_name', 'uid']))
@click.option('--projects',
              '-p',
              multiple=True,
              help='Filter by project names.')
...
@click.option('--past-days',
              ...
              default=1,
              type=float,
              help='Include only the past N days')
@click.option('--plot/--no-plot',
              default=True,
              help='Show the bar plot or not.')
@click.option('--csv',
              is_flag=True,
              default=False,
...
def throughput(db_obj, group_by, past_days, projects, seeds, aggregate, plot,
    ...
    import sys
    if atomate:
        dataframe = db_obj.throughput_summary_atomate(past_days=past_days,
                                                      projects=projects,
                                                      seeds=seeds,
                                                      aggregate=aggregate,
                                                      group_by=group_by,
        ...
        dataframe = db_obj.throughput_summary(past_days=past_days,
                                              projects=projects,
                                              seeds=seeds,
                                              aggregate=aggregate,
                                              group_by=group_by,
    ...
    if csv:
        ...
        with pd.option_context('display.max_rows', 99999,
...
def get_launch_info(fws, query):
    ...
    fw_info = namedtuple(
        'FWInfo',
        ['fw_id', 'seed_name', 'project_name', 'state', 'launch_dir'])
    ...
    for tmp in res:
        if tmp['launches']:
            ...
            launch_dir = None
        output.append(
            fw_info(seed_name=tmp['spec']['seed_name'],
...
@click.option('--pull-mapping',
              help='A dictionary maps user names to remote machines names.')
@common_query_options
@pass_db_obj
...
@click.option('--cycles',
              type=int,
              help='Expected ionic steps for computing the projected time.')
@pass_db_obj
...
@click.option('--struct-name',
              required=False,
              help='Select strucures by regex')
@click.option('--include-init-structure/--no-include-init-structure',
              default=False)
@click.option('--include-seed/--no-include-seed', default=False)
@click.option('--include-param/--no-include-param', default=False)
@pass_db_obj
def retrieve_project(db_obj, project, seed, struct_name,
                     include_init_structure, include_seed, include_param):
    """
    ...
    for res in tqdm(results, total=total):
        ...
        if res.param_file:
            Path(res['struct_name'] + '.param').write_text(
                res.param_file.content)
        if res.init_structure_file:
            Path(res['struct_name'] + '-orig.cell').write_text(
                res.init_structure_file.content)
        if res.seed_file:
            Path(res['struct_name'] + '-seed.cell').write_text(
                res.seed_file.content)
