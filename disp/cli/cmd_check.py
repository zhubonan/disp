...
import click
from fireworks.core.launchpad import LAUNCHPAD_LOC, LaunchPad

from disp.database.api import DB_FILE, ResFile, SearchDB
from disp.scheduler import Dummy, Scheduler
...
@click.option("--allow-dummy", is_flag=True, default=False, type=bool, help="Allow dummpy scheduler")
...
def airss():
    ...
    for name in ["buildcell", "cabal", "castep_relax"]:
        try:
            loc = subprocess.check_output(["which", "cabal"], universal_newlines=True).strip()
...
def database(ctx, past_days):
    ...
    pipeline = [
        {"$match": {"created_on": {"$gt": cutoff}}},
        {"$group": {"_id": {"project": "$project_name"}, "count": {"$sum": 1}}},
    ]
