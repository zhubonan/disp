...
@click.option(
    ...
    help=
    'Executable to be used, including the mpi runner part. The latter may be overridden by the worker.'
)
@click.option(
    '--modcell',
    type=str,
...
@click.option('--keep/--no-keep',
              default=True,
              help='Keep intermediate files, e.g. do no clean the workding directory.')
@click.option('--dryrun', is_flag=True)
@click.option(
    '--castep-code',
    show_default=True,
...
@click.option('--cluster', is_flag=True, default=False)
@pass_lpad
def deploy_singlepoint(lpad, code, seed, cell, base_cell, param, project, exe,
                 keep, dryrun, priority, category, record_db,
...
def apply_modcell(modcell, cell_content):
    ...
    if len(tokens) == 2:
        ...
        raise ValueError(
            '`modcell` should be in the format of `filename:func`.'
        )
    # Apply the function
    cell_content = '\n'.join(modfunc(cell_content.split('\n')))
    return cell_content
