...
@click.option('--disp-additional-field',
              multiple=True,
              help='Additional field to include.')
@click.option('--fw-additional-field',
              multiple=True,
              help='Additional field to include for fireworks.')
@click.option(
    '--wf-additional-field',
    multiple=True,
    help='Additional field to include for fireworks\'s workflow collection.')
@pass_db_obj
def build_index(sdb, disp_additional_field, fw_additional_field,
                wf_additional_field):
    """
    ...
    sdb.build_indexes(additional_fields=disp_additional_field,
                      wf_additional_fields=wf_additional_field,
                      fw_additional_fields=fw_additional_field)
...
def update_spec(sdb, seed_name, project_name, spec_update, confirm):
    ...
    if confirm:
        ok = click.confirm(
            f'Update {len(fw_ids)} fireworks with {spec_update}?', abort=True)
    else:
        ok = True
    if ok:
...
def delete_data(sdb, seed, project, struct, commit):
    ...
    project_names_to_delete = set(x.project_name
                                  for x in ResFile.objects(**disp_query))
    seed_names_to_delete = set(x.seed_name
                               for x in ResFile.objects(**disp_query))

    click.echo(f"Number of workflows to be deleted: {len(wf_ids)}")
    click.echo(
        f"Number of SHELX entries and initial structures to be deleted: {ndisp_entries}/{ndisp_entries_init}"
    )
    click.echo(
        f"Deletion involves seeds: {seed_names_to_delete}, projects: {project_names_to_delete}"
    )

    if commit:
        ok = click.confirm(
            f"Are you sure you want to delete the data?\nThis operation cannot be reversed!",
            abort=True)
        if ok:
            for wf_id in wf_ids:
