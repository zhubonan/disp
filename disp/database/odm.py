...
__all__ = ("Creator", "DispEntry", "SeedFile", "ResProperty", "ResFile", "InitialStructureFile", "ParamFile")
...
class DispEntry(Document):
    ...
    content = StringField(required=True)  # Raw string field for the "content" of the entry. This maps to a small file.
...
class SeedFile(DispEntry):
    ...
    md5hash = StringField(max_length=40, required=True, unique=False)  # Require MD5 hash - used for deduplicate
...
class ParamFile(DispEntry):
    ...
    md5hash = StringField(max_length=40, required=True, unique=False)  # MD5 hash for deduplicate
