...
def parse_param(seed):
    ...
    with open(seed + '.param', 'r') as seedfile:
...
def RASH_prepare_seed(seed, relaxed, amp):
    ...
    rlx_lattice = trim_stream(rlx_cell, r'^%BLOCK [Ll][Aa][Tt]',
                              r'^%ENDBLOCK [Ll][Aa][Tt]', 'FIX_VOL',
                              'ANG').read()
    rlx_lattice = rlx_lattice.replace('%ENDBLOCK', '#FIX\nENDBLOCK')
    # Get the position block
    rlx_pos = trim_stream(rlx_cell, r'^%BLOCK [Pp][Oo][Ss]',
                          r'^%ENDBLOCK [Pp][Oo][Ss]').read()
    # Preserve what is in between two blocks
    seed_rest = filter_out_stream(seed_cell, r'^%BLOCK [Ll][Aa][Tt]',
                                  r'^%ENDBLOCK [Ll][Aa][Tt]')
    # Take out the pos block and get rid of unwanted AIRSS parameters
    seed_rest = filter_out_stream(seed_rest, r'^%BLOCK [Pp][Oo][Ss]',
                                  r'^%ENDBLOCK [Pp][Oo][Ss]', '#POSAMP=',
                                  '#SYMMOPS=', '#NFORM=', '#SUPER').read()
...
def extract_result(seed):
    ...
    """
    results = {}
    with open(seed + '.history', 'r') as res_file:
    ...
    if 'hl' not in results or 'cl' not in results:
        ...
        raise RuntimeError(
            'Energy or cell volume not found in {}.history'.format(seed))
...
def write_converge(seed, suffix='castep'):
    ...
    for i, value, iter_num, timer in zip(num, conv_info['H'],
                                         conv_info['iter_num'],
                                         conv_info['time']):
        out_file.write('{:<5d}{:<20.5f}{:<10d}{:.2f}\n'.format(
            i, value, iter_num, timer))
    out_file.close()
...
def push_cell(cellout, cell):
    ...
    """
    with open(cellout) as fhout, open(cell) as fcell, open(
            str(cell) + '.tmp', 'w') as ftmp:

        # get the new lattice from the out cell
        new_lattice = trim_stream(fhout, r'^%BLOCK [Ll][Aa][Tt]',
                                  r'^%ENDBLOCK [Ll][Aa][Tt]',
                                  ['FIX_VOL', 'ANG'])
        new_pos = trim_stream(fhout, r'^%BLOCK [Pp][Oo][Ss]',
                              r'^%ENDBLOCK [Pp][Oo][Ss]')
        ftmp.write(new_lattice.read())
        ftmp.write('\n')
        ftmp.write(new_pos.read())
        # Do it twice to preserve what is in between two blocks
        old_rest = filter_out_stream(fcell, r'^%BLOCK [Ll][Aa][Tt]',
                                     r'^%ENDBLOCK [Ll][Aa][Tt]')
        old_rest = filter_out_stream(old_rest, r'^%BLOCK [Pp][Oo][Ss]',
                                     r'^%ENDBLOCK [Pp][Oo][Ss]')
