...
class DataCollector:
    ...
    """
    BASE_PROJECTION = [
        'struct_name', 'output.forces', 'input.incar', 'input.hubbards',
        'input.structure', 'output.energy_per_atom', 'output.energy',
        'output.structure', 'output.stress', 'seed_name', 'state',
        'project_name', 'input.xc_override', 'input.pseudo_potential', 'uuid',
        'unique_name', 'completed_at', 'task_id'
    ]
    ...
    def collect(self):
        ...
        for entry in docs:
            ...
            incar_entries = {
                key.lower(): value
                for key, value in entry['input']['incar'].items()
            }
            ...
            potcar_mapping = dict(
                zip(poscar.site_symbols,
                    entry['input']['pseudo_potential']['labels']))
            entry_dict = {
                'label':
                entry['struct_name'],
                'struct_name':
                ...
                entry['seed_name'],
                'umap':
                entry['input']['hubbards'],
                'functional':
                entry['input']['xc_override'],
                'pmg_struct_relaxed':
                structure,
                'pmg_struct':
                input_structure,
                ...
                entry['input']['pseudo_potential']['functional'],
                'potential_mapping':
                potcar_mapping,
                'uuid':
                entry['uuid'],
...
def get_entry(dataframe: pd.DataFrame,
              pmg_col='pmg_struct',
              label_col='label',
              uuid_col='uuid',
              umap_col='umap',
    ...
    for idx, row in dataframe.iterrows():
        ...
        attrs = {
            ...
            'dataframe_idx':
            idx,  # Store the entry index in the original dataframe
        }
        pd_entries.append(
...
def export_dataframe_as_res(dataframe: pd.DataFrame,
                            comment: str = 'VASP export',
                            extra_comments: list = None,
                            stress_key=None):
    """
    ...
    for _, row in dataframe.iterrows():
        ...
        res = RESFile(
            ...
            {
                'enthalpy':
                row.energy_per_atom * len(relaxed.sites),
                'volume':
                row.volume_per_fu * row.nform_refine,
                # The stress data may not existsk
                'pressure':
                0.0 if stress_key in row.index or stress_key is None else
                row[stress_key],
                'label':
                row.label,
                'rem':
                comments + [
                    '{} = {}'.format(key, row[key])
                    for key in row.index if 'struct' not in key
                ]
            })
        content = '\n'.join(res.to_res_lines())
        Path('exports/{}.res'.format(row.label)).write_text(content)
...
def deposit_into_aiida(row, group, dryrun):
    ...
    from aiida.engine import calcfunction  # pylint: disable=import-outside-toplevel
    from aiida.orm import StructureData, Dict  # pylint: disable=import-outside-toplevel
