...
def get_relax_wf_std(
    ...
    if isinstance(structure, RESFile):
        ...
        raise ValueError(f"Unsupported structure input: {type(structure)}")

    if name is None:
        raise RuntimeError("Must give a name for the structure for provenance")

    kpoints = Kpoints(kpts=(calc_kpt_tuple_recip(structure, kpoints_spacing),))
    ...
    if "ISMEAR" in incar_overrides:
        ...
        incar_modify = {
            "incar_update": ">>incar_update<<",  # Taking incar update from the worker specific settings
            "incar_dictmod": {"_set": {"ISMEAR": incar_overrides.get("ISMEAR")}},
        }
    ...
    # INCAR is invalid.....
    wfl = add_modify_incar(wfl, modify_incar_params={"incar_update": {"ICHARG": 2}}, fw_name_constraint="STATIC")
    ...
    if not metadata:
        warn("No metadata is set - workflow identification solely relying on name of the structure")
        unique_name = None
    else:
        ...
        unique_name = f"{project_name}:{seed_name}:{struct_name}"
...
def get_incar_overrides_base():
    ...
    base_overrides = {
        ...
        "LDAU": True,
        "LDAUJ": {
            "F": {"Co": 0, "Cr": 0, "Fe": 0, "Mn": 0, "Mo": 0, "Ni": 0, "V": 0, "W": 0, "Cu": 0},
            "O": {"Co": 0, "Cr": 0, "Fe": 0, "Mn": 0, "Mo": 0, "Ni": 0, "V": 0, "W": 0, "Cu": 0},
        },
        "LDAUL": {
            "F": {"Co": 2, "Cr": 2, "Fe": 2, "Mn": 2, "Mo": 2, "Ni": 2, "V": 2, "W": 2, "Cu": 2},
            "O": {"Co": 2, "Cr": 2, "Fe": 2, "Mn": 2, "Mo": 2, "Ni": 2, "V": 2, "W": 2, "Cu": 2},
        },
        "LDAUTYPE": 2,
        "LDAUPRINT": 1,
        "GGA": "PE",
        "LDAUU": {
            "F": {"Co": 3.32, "Cr": 3.7, "Fe": 4.0, "Mn": 3.9, "Mo": 4.38, "Ni": 6.2, "V": 3.25, "W": 6.2, "Cu": 5.17},
            "O": {"Co": 3.32, "Cr": 3.7, "Fe": 4.0, "Mn": 3.9, "Mo": 4.38, "Ni": 6.2, "V": 3.25, "W": 6.2, "Cu": 5.17},
        },
        "LORBIT": 11,
        "LREAL": False,
        "LWAVE": False,
