...
class AirssBuildcellTask(FiretaskBase):
    ...
    optional_params = ["upload", "store_content", "keep_seed", "deposit_init_structure", "build_timeout"]
    logger = get_fw_logger(__name__, l_dir=None, stream_level="INFO")
...
class AirssModcellTask(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        if func_content:
            ...
            funcname = self["func"]
            mod = __import__("modfunc", globals(), locals(), [str(funcname)], 0)
...
class AirssCastepRelaxTask(FiretaskBase):  # pylint: disable=too-many-instance-attributes
    ...
    optional_params = ["minimum_run_time", "prepend_command", "append_command", "castep_code", "cluster"]
    ...
    def _init_parameters(self, fw_spec):
        ...
        self.minimum_run_time = self.get("minimum_run_time", self.MINIMUM_RUN_TIME)
        ...
        if self.castep_code != "default":
            if self.castep_code in codes:
                ...
                raise RuntimeError(f"Required '{self.castep_code}' code is not found in the worker environment!")
        ...
        self.prepend_command = self.get("prepend_command", fw_env.get(f"{self.code_string}_prepend_command"))
        self.append_command = self.get("append_command", fw_env.get(f"{self.code_string}_append_command"))
        ...
        if mpicmd:
            ...
            for idx, token in enumerate(tokens):
                if "castep" in token:
                    ...
                    break
            if istart is None:
                raise RuntimeError(f"CASTEP executable cannot be detected in {exe}")

            # Take everything after castep after the launcher command
            exe = "{} {}".format(mpicmd, " ".join(tokens[istart:]))

        cmd = ["castep_relax", f"{self.cycles:d}", exe, "0", "0", self.struct_name]
        return cmd
    ...
    def _handle_relax_finshed(self):
        ...
        self._save_res()
        return FWAction(
            stored_data={"relax_status": RelaxOutcome.FINISHED.name, "struct_name": self.struct_name, "seed_name": self.seed_name}
        )

    def _handle_too_few_cycle_left(self, rem_cycles):
        """Handle the case where there are too few cycles left"""

        self.logger.info(f"Only have {rem_cycles} to perform, relaxation terminated")
    ...
    def run_task(self, fw_spec):
        ...
        run_files = [self.struct_name + suffix for suffix in [".cell", ".param", ".castep"]]
        ...
        with create_symlinks(base_path, self.project_name, run_files):
            relax_outcome = self._run_relax(
                self.struct_name, cmd, actual_timeout, prepend_command=self.prepend_command, append_command=self.append_command
            )
        ...
        if relax_outcome is RelaxOutcome.TIMEDOUT:
            ...
            if (self.cycles != 0) and (reminaing_cycles < 20):
                return self._handle_too_few_cycle_left(rem_cycles=reminaing_cycles)
    ...
    def _handle_insufficient_run_time(self, fw_spec):
        ...
        self.logger.warning("Insufficient run time detected. This should never happen if the timeout is set properly for the worker....")
    ...
    def _handle_relax_timeout(self, fw_spec):
        ...
        if has_spin:
            ...
            subprocess.run(f"cabal castep cell < {struct_name}.castep > cell_update.cell", shell=True, check=True)
    ...
    def _handle_relax_error(self, fw_spec):
        ...
        return FWAction(stored_data=stored_data, update_spec={"relax_status": relax_status})
    ...
    def _save_res(self):  # pylint: disable=no-self-use
        """Save the res file"""
        proc = subprocess.run(["castep2res", str(self.cluster), self.struct_name], capture_output=True, text=True, check=True)
    ...
    def _upload_dot_castep(self):
        ...
        try:
            self.search_db.upload_dot_castep(self.struct_name, self.seed_name, self.project_name)
            self.logger.info("Uploaded .castep file to the database")
            return True
        except FileExistsError:
            self.logger.error("Cannot upload the .castep to the database - file exists")
            return False

    def _retrieve_dot_castep(self):
        """Retrieve the .castep file"""
        try:
            self.search_db.retrieve_dot_castep(self.struct_name, self.seed_name, self.project_name)
            self.logger.info("Retrieved .castep file to the database")
            return True
        except FileNotFoundError:
            self.logger.error("the .castep from the previous calculation cannot be found.")
            return False

    def _delete_dot_castep(self):
        """Retrieve the .castep file"""
        self.logger.info("Deleted .castep file in the database")
        self.search_db.delete_dot_castep(self.struct_name, self.seed_name, self.project_name)

    def _run_relax(self, struct_name, cmd, timeout, prepend_command=None, append_command=None):
        ...
        with transient_file(".realx_stdout", mode="w+") as tmpout:
            try:
                subprocess.run(f"./{self.run_script_name}", timeout=timeout, check=True, text=True, stdout=tmpout)
            except subprocess.TimeoutExpired:
                ...
                if "Pressure" in stdout_content and castep_finish_ok(struct_name + ".castep"):
    ...
    def _prepare_run_script(self, cmd, prepend_command=None, append_command=None):
        """
...
"""
        ...
        # Set the permission
        subprocess.run(f"chmod +x {self.run_script_name}", shell=True, check=True)
...
class AirssGulpRelaxTask(AirssCastepRelaxTask):
    ...
    def _get_cmd(self):
        ...
        # zero pressure
        cmd = ["gulp_relax", self.executable, str(self.cluster), str(self.pressure), self.struct_name]
        return cmd
    ...
    def _run_relax(self, struct_name, cmd, timeout, prepend_command=None, append_command=None):
        ...
        with transient_file(".relax_stdout", mode="w+") as outtmp:
            try:
                subprocess.run(f"./{self.run_script_name}", timeout=timeout, check=True, text=True, stdout=outtmp)
                ...
                outtmp.seek(0)
                content = outtmp.read()
                if gulp_relax_finish_ok(struct_name + ".castep") and "Volume" in content:
                    outcome = RelaxOutcome.FINISHED
                else:
                    outcome = RelaxOutcome.ERRORED
    ...
    def _handle_relax_timeout(self, fw_spec):
        ...
        new_task = self.__class__(param_content=self["param_content"], cycles=self.cycles, executable=self["executable"])
...
class AirssPp3RelaxTask(AirssGulpRelaxTask):
    ...
    def _get_cmd(self):
        ...
        # NOTE: this assumes that we are working with a periodic system and
        # zero pressure
        cmd = ["pp3_relax", self.executable, self.struct_name]
        return cmd
...
class AirssDataTransferTask(FiretaskBase):
    ...
    optional_params = ["keep", "additional_types", "base_path", "additional_files"]
    ...
    def run_task(self, fw_spec):  # pylint: disable=too-many-branches
        ...
        types_to_transfer = self.DEFAULT_TYPES + self._get_param(fw_spec, "additional_types", [])
        ...
        if "seed_name" in fw_spec:
            ...
            if seed_file.is_file():
                ...
                if seed_file_pfolder.is_file():
                    ...
                    if not filecmp.cmp(seed_file.resolve(), seed_file_pfolder.resolve()):
                        # Two seeds are not the same - something is wrong!!! Backup the seed as '-seed.cell'
                        self.logger.warning("Found seed in the project folder, but it is NOT THE SAME as used here!")
                        shutil.copy2(seed_file, pfolder / (struct_name + "-seed.cell"))
                else:
