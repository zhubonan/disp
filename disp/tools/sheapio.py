    ...
    def __init__(self, params):
        ...
        for key, value in params.items():
            if key not in self._default:
                raise ValueError(f"Key: {key} is not a valid input option for SHEAP")
...
SheapMetadata = namedtuple(
    "SheapMetadata", ("label", "natoms", "form", "sym", "volume", "enthalpy", "nfound"), defaults=("SHEAP-IN", 1, "Al", "P1", -0.1, -0.1, 1)
)


def write_sheap_vec(handle, vecs: Union[np.array, List[np.array]], metadata=None):
    """
...
SheapOut = namedtuple("SheapOut", ("labels", "nforms", "form", "sym", "volume", "enthalpy", "nfound", "radius", "coords", "nitems", "cost"))
...
def run_sheap(
    ...
    with Popen(cmdline, stdin=PIPE, stdout=PIPE, universal_newlines=True, stderr=stderr) as popen:
        # Write to stdin
...
def parse_sheap_output(indata) -> SheapOut:
    ...
    return SheapOut(labels, nforms, form, sym, volume, enthalpy, nfound, radius, coords, nitems, cost)
...
def get_sheap_metadata(dataframe: pd.DataFrame) -> List[SheapMetadata]:
    ...
    for _, row in dataframe.iterrows():
        metadata.append(
            SheapMetadata(
                label=row.label, natoms=row.natoms, form=row.reduced_formula, enthalpy=row.enthalpy, sym=row.symm, volume=row.volume
            )
        )
    return metadata
...
def sheap_to_dict(sheapout: SheapOut, structure_path: Union[dict, str, Path]):
    ...
    outdict = {
        "meta": {"name": "SHEAP", "description": "Map generated with SHEAP"},
    }
    outdict["properties"] = {}
    outdict["properties"]["volume"] = {"target": "structure", "values": sheapout.volume}
    outdict["properties"]["count"] = {"target": "structure", "values": sheapout.nfound}
    outdict["properties"]["radius"] = {"target": "structure", "values": sheapout.radius}
    # Compute the sizes
    sizes = np.array(sheapout.radius) ** 2 * np.pi
    outdict["properties"]["size"] = {"target": "structure", "values": sizes.tolist()}
    outdict["properties"]["energy"] = {"target": "structure", "values": sheapout.enthalpy}
    outdict["properties"]["label"] = {"target": "structure", "values": sheapout.labels}

    coords = sheapout.coords
    dims = len(sheapout.coords[0])
    for dim in range(dims):
        outdict["properties"][f"sheap{dim+1}"] = {"target": "structure", "values": [entry[dim] for entry in coords]}
    ...
    if spacegroups:
        outdict["properties"]["spacegroup"] = {"target": "structure", "values": spacegroups}
    return outdict
