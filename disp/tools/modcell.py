...
def replace_block(lines: List[str], block_name: str, block_pattern: str, new_value: List[str], check=True) -> List[str]:
    """
...
def modify_cell(base_cell: str, atoms: ase.Atoms):
    ...
    new_lines = replace_block(base_lines, "POSITIONS_ABS", "POSITIONS_(FRAC|ABS)", pos_lines)
    new_lines = replace_block(new_lines, "LATTICE_CART", "LATTICE_(CART|ABC)", cell_lines)
